...
namespace Hertzole.ALE
{
    public class PanelGroup : IPanelGroupElement
    {
        internal class InternalSettings
        {
            ...
            public void TryChangeSizeOf(IPanelGroupElement element, PanelDirection direction, float deltaSize) { group.TryChangeSizeOf(element, direction, deltaSize); }
            public void ResizeElementTo(IPanelGroupElement element, Vector2 newSize, PanelDirection horizontalDir, PanelDirection verticalDir) { group.ResizeElementTo(element, newSize, horizontalDir, verticalDir); }
        ...
        protected readonly PanelDirection direction;
        ...
        public PanelCanvas Canvas { get; private set; }
        ...
        protected virtual void UpdateBounds(Vector2 position, Vector2 size)
        {
            ...
            else
            {
                ...
                if (horizontal)
                {
                    multiplier = Size.x == 0f ? size.x : size.x / Size.x;
                }
                else
                {
                    multiplier = Size.y == 0f ? size.y : size.y / Size.y;
                }
        ...
        protected virtual void UpdateLayout()
        {
            if (isDirty)
            {
                ...
                for (int i = 0; i < elements.Count; i++)
                {
                    ...
                    if (elements[i] is Panel panel && panel.Internal.IsDummy)
                    {
                        dummyPanelIndex = i;
                    }
                }
        ...
        protected void ResizeElementTo(IPanelGroupElement element, Vector2 newSize, PanelDirection horizontalDir, PanelDirection verticalDir)
        {
            if (horizontalDir != PanelDirection.Left && horizontalDir != PanelDirection.Right)
            {
                horizontalDir = PanelDirection.Right;
            }

            if (verticalDir != PanelDirection.Bottom && verticalDir != PanelDirection.Top)
            {
                verticalDir = PanelDirection.Bottom;
            }

            PanelDirection horizontalOpposite = horizontalDir.Opposite();
            PanelDirection verticalOpposite = verticalDir.Opposite();
