...
{
    [CustomEditor(typeof(LevelEditorInputSystem))]
    public class LevelEditorInputSystemEditor : UnityEditor.Editor
    {
#if !OBSOLETE
        ...
        public override void OnInspectorGUI()
        {
            ...
            serializedObject.ApplyModifiedProperties();
#else
            if (GUILayout.Button("Replace with Level Editor Input"))
            {
                GameObject go = ((LevelEditorInputSystem)target).gameObject;

                Undo.DestroyObjectImmediate(go.GetComponent<LevelEditorInputSystem>());
                Undo.AddComponent<LevelEditorInputSystem>(go);
            }
#endif
        }
            ...
            if (actionAsset.objectReferenceValue == null)
            {
                ...
                if (GUILayout.Button("Create Actions", GUILayout.Width(140)))
                {
                    ...
                    if (!string.IsNullOrEmpty(fileName))
                    {
                        ...
                        InputActionMap levelEditorMap = new InputActionMap("Level Editor");
                        actions.AddActionMap(levelEditorMap);
